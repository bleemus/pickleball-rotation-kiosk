...
describe('useApi', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('throws error when getSession fails', async () => {
      ...
      await expect(
        result.current.getSession('invalid-id')
      ).rejects.toThrow();
    });
    ...
    it('throws error when deleteSession fails', async () => {
      ...
      await expect(
        result.current.deleteSession('session-123')
    ...
    it('throws error when addPlayer fails', async () => {
      ...
      await expect(
        result.current.addPlayer('session-123', 'Alice')
    ...
    it('throws error when completeRound fails', async () => {
      ...
      await expect(
        result.current.completeRound('session-123', [])
