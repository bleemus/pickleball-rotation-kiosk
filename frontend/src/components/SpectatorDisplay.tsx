...
export function SpectatorDisplay({ apiUrl }: SpectatorDisplayProps) {
  ...
  const [networkInfo, setNetworkInfo] = useState<{
    ...
    ip: string | null;
    port: string;
    allIPs?: { interface: string; ip: string }[];
  } | null>(null);
  ...
  if (isMobile) {
    ...
    return (
      <div
        className={`h-screen flex items-center justify-center p-6 ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-purple-500 to-blue-600"}`}
      >
        <div
          className={`rounded-3xl shadow-2xl p-8 max-w-lg text-center ${darkMode ? "bg-gray-800" : "bg-white"}`}
        >
          ...
          <p className={`text-lg mb-6 ${darkMode ? "text-gray-300" : "text-gray-600"}`}>
            The spectator view is designed for larger displays like TVs and monitors. Please use the
            main entry screen to manage games on mobile devices.
          </p>
          <a
            href={entryUrl}
  ...
  if (error && error !== "No active game session") {
    return (
      <div
        className={`h-screen flex items-center justify-center p-8 ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-red-500 to-pink-600"}`}
      >
        <div
          className={`rounded-3xl shadow-2xl p-12 text-center ${darkMode ? "bg-gray-800" : "bg-white"}`}
        >
          <h1 className={`text-4xl font-bold mb-4 ${darkMode ? "text-red-400" : "text-red-600"}`}>
            Error
          </h1>
  ...
  if (!session || error === "No active game session") {
    ...
    if (!networkInfo) {
      return (
        <div
          className={`h-screen flex items-center justify-center ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-purple-500 to-blue-600"}`}
        >
    ...
    return (
      <div
        className={`h-screen flex items-center justify-center p-8 ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-purple-500 to-blue-600"}`}
      >
        {/* WiFi QR Code - Top Left */}
        {wifiQRString && (
          <div className="fixed top-4 left-4 z-50 space-y-3">
            <div
              className={`rounded-xl shadow-2xl p-5 ${darkMode ? "bg-gray-800 border-2 border-blue-500" : "bg-white border-2 border-blue-400"}`}
            >
              <div className="flex flex-col items-center">
                <p
                  className={`text-lg font-bold mb-2 text-center ${darkMode ? "text-blue-400" : "text-blue-600"}`}
                >
                  Step 1: Connect to WiFi
                </p>
                <QRCode value={wifiQRString} size={150} level="M" />
                <p
                  className={`text-base mt-2 text-center font-bold ${darkMode ? "text-gray-200" : "text-gray-800"}`}
                >
                  {wifiSSID}
                </p>
                <p
                  className={`text-xs mt-1 text-center ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                >
                  Scan to connect
                </p>
              </div>
        ...
        <div
          className={`rounded-3xl shadow-2xl p-12 max-w-4xl text-center ${darkMode ? "bg-gray-800" : "bg-white"}`}
        >
          <h1 className={`text-5xl font-bold mb-6 ${darkMode ? "text-gray-200" : "text-gray-800"}`}>
            Welcome to {APP_NAME}
          </h1>
          ...
          {isLocalhost && urlSource === "network" && (
            <div
              className={`mb-6 p-4 border-2 rounded-xl ${darkMode ? "bg-blue-900 border-blue-500" : "bg-blue-50 border-blue-400"}`}
            >
              <p
          ...
          <div
            className={`mb-8 p-6 rounded-2xl shadow-lg ${darkMode ? "bg-gray-700" : "bg-gradient-to-r from-green-500 to-blue-600"}`}
          >
            ...
            <div className="bg-white rounded-xl p-6 shadow-inner mb-4 flex justify-center">
              <QRCode value={baseUrl} size={200} level="M" />
            </div>
          ...
          <div className="text-left space-y-4 mb-8">
            ...
            <ol
              className={`list-decimal list-inside space-y-3 text-xl ml-4 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
            >
  ...
  if (session?.ended) {
    ...
    return (
      <div
        className={`h-screen flex flex-col ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-purple-600 to-blue-600"}`}
      >
        {/* Fixed Header */}
                  ...
                  {sortedPlayers.map((player, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className={`py-6 px-4 text-2xl text-center ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                        >
                          {player.gamesPlayed}
                        </td>
                        <td
                          className={`py-6 px-4 text-2xl text-center font-bold ${
  ...
  if (!networkInfo) {
    return (
      <div
        className={`h-screen flex items-center justify-center ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-green-500 to-blue-600"}`}
      >
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-white mx-auto mb-4"></div>
          <p className="text-white text-xl font-semibold">Loading...</p>
  ...
  return (
    <div
      className={`h-screen flex ${darkMode ? "bg-gray-900" : "bg-gradient-to-br from-green-500 to-blue-600"}`}
    >
      {/* Top Left - App URL QR Code only (no WiFi QR since user is already connected) */}
      <div className="fixed top-4 left-4 z-50">
        <div
          className={`rounded-xl shadow-2xl p-3 ${darkMode ? "bg-gray-800 border-2 border-green-500" : "bg-white border-2 border-green-400"}`}
        >
          <div className="flex flex-col items-center">
            <p
              className={`text-sm font-bold mb-1 text-center ${darkMode ? "text-green-400" : "text-green-600"}`}
            >
              Scan to Connect
            </p>
            <QRCode value={baseUrl} size={100} level="M" />
            <p
              className={`text-xs mt-1 text-center ${darkMode ? "text-gray-400" : "text-gray-600"}`}
            >
              Open app
      ...
      <div className="flex-1 p-6 flex flex-col overflow-hidden">
        ...
        <div className="text-center mb-6 flex-shrink-0">
          {hasActiveRound && round ? (
            <>
              <h1
                className={`text-6xl font-bold mb-2 ${darkMode ? "text-gray-200" : "text-white"}`}
              >
        ...
        {hasActiveRound && round ? (
          <>
            <div
              className={`grid ${getGridCols(round.matches.length)} gap-6 mb-6 flex-1 min-h-0 overflow-auto`}
            >
                ...
                return (
                  ...
                  >
                    <div className="text-center mb-4 flex-shrink-0">
                      <h2
                        className={`font-bold ${getCourtTitleSize(round.matches.length)} ${darkMode ? "text-gray-200" : "text-gray-800"}`}
                      >
                    ...
                    <div className="space-y-4 flex-1 flex flex-col justify-center px-8 lg:px-16 xl:px-24">
                      ...
                      >
                        ...
                        {match.completed && (
                          <div
                            className={`ml-4 flex-shrink-0 w-40 text-center font-bold ${getScoreSize(round.matches.length)} ${
                              team1Won
                                ? "text-green-400"
                                : darkMode
                                  ? "text-gray-400"
                      ...
                      <div className="text-center flex-shrink-0">
                        <span
                          className={`font-bold ${getVsSize(round.matches.length)} ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                        >
                          VS
                        </span>
                      </div>
          ...
          <div
            className={`grid ${getGridCols(previousRoundHistory.length)} gap-6 mb-6 flex-1 min-h-0 overflow-auto`}
          >
            {previousRoundHistory.map((game) => {
              ...
              return (
                ...
                >
                  <div className="text-center mb-4 flex-shrink-0">
                    <h2
                      className={`font-bold ${getCourtTitleSize(previousRoundHistory.length)} ${darkMode ? "text-gray-200" : "text-gray-800"}`}
                    >
                  ...
                  <div className="space-y-4 flex-1 flex flex-col justify-center px-24">
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-bold text-center break-words ${getPlayerNameSize(previousRoundHistory.length)} ${darkMode ? "text-gray-200" : "text-gray-800"}`}
                        >
                          {game.team1Players[0]}
                        </p>
                        <p
                      ...
                      <div
                        className={`font-bold ml-4 flex-shrink-0 w-40 text-center ${getScoreSize(previousRoundHistory.length)} ${team1Won ? "text-green-400" : darkMode ? "text-gray-400" : "text-gray-700"}`}
                      >
                        {game.team1Score}
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div
                        className={`font-bold ml-4 flex-shrink-0 w-40 text-center ${getScoreSize(previousRoundHistory.length)} ${team2Won ? "text-green-400" : darkMode ? "text-gray-400" : "text-gray-700"}`}
                      >
                        {game.team2Score}
                      </div>
                    </div>
