            ...
            <div className="bg-white rounded-3xl shadow-2xl p-6 lg:p-10 max-w-5xl w-full max-h-full overflow-y-auto">
                ...
                <div className="mb-6 lg:mb-8">
                    <h2 className="text-xl lg:text-2xl font-semibold mb-3 lg:mb-4 text-gray-700">
                        ...
                        {players.filter((p) => !p.forceSitOut).length} Active,{" "}
                        {players.filter((p) => p.forceSitOut).length} Sitting
                    </h2>

                    {players.length === 0 ? (
                        <p className="text-gray-500 text-base lg:text-lg italic">
                        ...
                        if (players.length < minRequired) {
                            return (
                                <p className="text-orange-600 mt-3 text-sm lg:text-base font-medium">
                                    ⚠️ Need at least {minRequired} players to
                                    start a round ({numCourts}{" "}
                                    {numCourts === 1 ? "court" : "courts"} × 4
                                    players)
                                </p>
                            );
                        }
                    ...
                    {(() => {
                        const sittingCount = players.filter(
                            (p) => p.forceSitOut,
                        ).length;
                        ...
                        if (sittingCount > 0 && availableCount < minRequired) {
                            return (
                                <p className="text-red-600 mt-3 text-sm lg:text-base font-bold bg-red-100 border-2 border-red-400 px-4 py-3 rounded-xl">
                                    ⚠️ Not enough players available.{" "}
                                    {sittingCount} player(s) marked to sit out,
                                    need at least {minRequired} available
                                    players.
                                </p>
                            );
                        }
