...
export class GamePage {
  ...
  async enterScoreForMatch(matchIndex: number, team1Score: number, team2Score: number) {
    ...
    if (!team1Input || !team2Input) {
      throw new Error(`Could not find inputs for match ${matchIndex}. Found ${visibleInputs.length} visible inputs.`);
    }
  ...
  async enterScoresMode() {
    ...
    await this.page.locator('input[type="number"]').first().waitFor({ state: 'attached', timeout: 5000 });
  }
